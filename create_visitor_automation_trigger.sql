-- =================================================================
-- VISITOR AUTOMATION TRIGGER
-- Automatically creates/updates visitors when events are inserted
-- =================================================================
...
FOR EACH ROW
BEGIN
    -- Only process if UUID exists and is valid
    IF NEW.uuid IS NOT NULL AND NEW.uuid != '' AND NEW.uuid != 'null' THEN
        ...
        IF EXISTS (SELECT 1 FROM superpixel_visitors WHERE uuid = NEW.uuid) THEN
            ...
            -- Important: Preserve business_email if it exists, only update "last" fields with non-empty values
            UPDATE superpixel_visitors 
            SET
                -- Update basic info (only if current is empty)
                first_name = COALESCE(NULLIF(first_name, ''), NEW.first_name),
                last_name = COALESCE(NULLIF(last_name, ''), NEW.last_name),
                
                -- Update personal info (only if current is empty)
                ...
                -- Demographics (only if current is empty)
                ...
                -- Phone numbers (only if current is empty)
                ...
                -- Emails - SPECIAL HANDLING: Preserve business_email over personal
                business_email = CASE 
                ...
                -- Job info (only if current is empty)
                ...
                -- Company info (only if current is empty)
                ...
                -- Social info (only if current is empty)
                ...
                -- Skiptrace data (only if current is empty)
                ...
                -- Valid phones (only if current is empty)
                valid_phones = COALESCE(NULLIF(valid_phones, ''), NEW.valid_phones),
                
                -- NPN/CRD (only if current is empty)
                npn = COALESCE(NULLIF(npn, ''), NEW.npn),
                crd = COALESCE(NULLIF(crd, ''), NEW.crd),
                
                -- IMPORTANT: "Last" Event Fields - Always update with latest non-empty values
        ...
        ELSE
            ...
            INSERT INTO superpixel_visitors (
                ...
                npn, crd, event_count, first_seen_at, last_seen_at
            ) VALUES (
                NEW.uuid, NEW.first_name, NEW.last_name, NEW.personal_address, NEW.personal_city, NEW.personal_state,
                NEW.personal_zip, NEW.personal_zip4, NEW.age_range, NEW.children, NEW.gender, NEW.homeowner, NEW.married,
                NEW.net_worth, NEW.income_range, NEW.direct_number, NEW.direct_number_dnc, NEW.mobile_phone,
                NEW.mobile_phone_dnc, NEW.personal_phone, NEW.personal_phone_dnc, NEW.business_email,
                NEW.personal_emails, NEW.deep_verified_emails, NEW.sha256_personal_email, NEW.sha256_business_email,
                NEW.hem_sha256, NEW.job_title, NEW.headline, NEW.department, NEW.seniority_level, NEW.inferred_years_experience,
                NEW.company_name_history, NEW.job_title_history, NEW.education_history, NEW.company_address,
                NEW.company_description, NEW.company_domain, NEW.company_employee_count, NEW.company_linkedin_url,
                NEW.company_name, NEW.company_phone, NEW.company_revenue, NEW.company_sic, NEW.company_naics,
                NEW.company_city, NEW.company_state, NEW.company_zip, NEW.company_industry, NEW.linkedin_url,
                NEW.twitter_url, NEW.facebook_url, NEW.social_connections, NEW.skills, NEW.interests,
                ...
                NEW.skiptrace_b2b_source, NEW.skiptrace_b2b_website, NEW.valid_phones,
                NEW.url, NEW.element, 
                ...
                NEW.npn, NEW.crd, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
            );
