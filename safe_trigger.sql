-- Safe trigger that avoids STR_TO_DATE errors
    ...
    IF NEW.uuid IS NOT NULL AND NEW.uuid != '' AND NEW.uuid != 'null' THEN
        IF EXISTS (SELECT 1 FROM superpixel_visitors WHERE uuid = NEW.uuid) THEN
            ...
            SET
                ...
                -- Update counters and timestamps - SAFE VERSION
                event_count = event_count + 1,
                last_seen_at = CURRENT_TIMESTAMP  -- Use database time for now, we'll fix this later
            WHERE uuid = NEW.uuid;
        ELSE
            -- Insert new visitor with ALL fields - SAFE VERSION
            INSERT INTO superpixel_visitors (
                ...
                1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP  -- Use database time for now
            );
        END IF;
    END IF;
END$$
...
SELECT 'Safe trigger created successfully!' as status; 