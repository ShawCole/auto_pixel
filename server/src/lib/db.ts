...
export async function ensureClientSchema(client: string) {
    ...
    try {
        ...
        log(`✅ Switched to database '${client}'`);
        ...
        log(`🔧 Creating procedures and triggers for '${client}'...`);
        await root.query(procedureAndTriggers);
        log(`✅ Successfully created procedures and triggers for '${client}'.`);
