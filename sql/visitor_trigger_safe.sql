            ...
            SET
                ...
                last_seen_at = CASE 
                  WHEN NEW.event_timestamp IS NOT NULL AND NEW.event_timestamp != '' 
                       AND NEW.event_timestamp REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}T'
                  THEN STR_TO_DATE(NEW.event_timestamp, '%Y-%m-%dT%H:%i:%sZ')
                  ELSE COALESCE(last_seen_at, CURRENT_TIMESTAMP)
                END
            WHERE uuid = NEW.uuid;
        ELSE
            INSERT INTO superpixel_visitors (
                ...
                CASE 
                  WHEN NEW.event_timestamp IS NOT NULL AND NEW.event_timestamp != '' 
                       AND NEW.event_timestamp REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}T'
                  THEN STR_TO_DATE(NEW.event_timestamp, '%Y-%m-%dT%H:%i:%sZ') ELSE CURRENT_TIMESTAMP END,
                CASE 
                  WHEN NEW.event_timestamp IS NOT NULL AND NEW.event_timestamp != '' 
                       AND NEW.event_timestamp REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}T'
                  THEN STR_TO_DATE(NEW.event_timestamp, '%Y-%m-%dT%H:%i:%sZ') ELSE CURRENT_TIMESTAMP END
