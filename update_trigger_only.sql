-- Update trigger to use event timestamps instead of database update time
-- This version only updates the trigger, doesn't try to fix historical data
    ...
    IF NEW.uuid IS NOT NULL AND NEW.uuid != '' AND NEW.uuid != 'null' THEN
        IF EXISTS (SELECT 1 FROM superpixel_visitors WHERE uuid = NEW.uuid) THEN
            ...
            SET
                ...
                last_seen_at = CASE 
                    WHEN NEW.event_timestamp IS NOT NULL 
                        AND NEW.event_timestamp != '' 
                        AND NEW.event_timestamp REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}'
...
SELECT 'Trigger updated with proper timestamp handling!' as status; 