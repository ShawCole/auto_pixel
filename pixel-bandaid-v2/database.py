        ...
        if not self.use_ssh:
             ...
             try:
                 with engine.connect() as conn:
                      query = text("SELECT client_name, pixel_name, sheet_id, client_website, segment_name, segment_api, on_platform_segment_url, on_platform_url, oplet, visitors, last_event_at, workspace, workspace_url FROM pixel_sheets WHERE paused = 0 AND oplet_polling_active = 1 AND client_name != 'VettaFi'")
                      ...
                      for row in result:
                          pixels.append({
                              ...
                              "on_platform_url": row[7],
                              "oplet": row[8],
                              "visitors": row[9],
                              "last_event_at": row[10],
                              "workspace": row[11],
                              "workspace_url": row[12]
                          })
    ...
    def _execute_update(self, conn_url, client_name, pixel_name, metadata):
        ...
        with engine.connect() as conn:
            ...
            if metadata.get("on_platform_segment_url"):
                ...
                updates.append("on_platform_url = :surl")
